...
module.exports.Image = function Image(document) {
...
module.exports.getImage = function (req,res,url) {
  new Image({ url: url }).convert (function (err, data) {
      if (err) {
        console.log(err);
      }
      if (data) {
        ...
        res.writeHead(200, {'Content-Type' : 'text/html','Content-Size' : data.length }); 
        res.write("error");
        res.end ();
      }
  });
